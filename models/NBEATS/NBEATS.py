...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.stack_types = [Model.GENERIC_BLOCK, Model.GENERIC_BLOCK]
        self.stacks = []
        self.parameters = []
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        for i in range(len(self.decomposition_variables)):
            for c in range(self.channels):
                x_c = x_enc[:,(i*self.channels)+c,:]
                backcast = x_c
                for stack_id in range(len(self.stacks)): # TODO: Check if stack + block is necessary for GenericNBEATS
