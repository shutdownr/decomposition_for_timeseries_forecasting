...
class PastDecomposableMixing(nn.Module):
    ...
    def forward(self, x_list):
        length_list = [x.shape[2] for x in x_list]
        ...
        for ori, out_season, out_trend, length in zip(x_list, out_season_list, out_trend_list,
            ...
            if self.channel_independence:
                out = ori + self.out_cross_layer(out.permute(0,2,1)).permute(0,2,1)
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.normalize_layers = torch.nn.ModuleList(
            [
                Normalize(self.configs.enc_in*len(self.decomposition_variables), affine=True, non_norm=True if configs.use_norm == 0 else False)
        ...
        if self.channel_independence:
            ...
            self.projection_layer = nn.Linear(
                configs.d_model, configs.c_out*len(self.decomposition_variables), bias=True)
    ...
    def __multi_scale_process_inputs(self, x_enc, x_mark_enc):
        if self.configs.down_sampling_method == "max":
            ...
            down_pool = nn.Conv1d(in_channels=self.configs.enc_in*len(self.decomposition_variables), out_channels=self.configs.enc_in*len(self.decomposition_variables),
    ...
    def future_multi_mixing(self, B, enc_out_list, x_list):
        ...
        if self.channel_independence:
            ...
            for i, enc_out in zip(range(len(x_list)), enc_out_list):
                ...
                dec_out = self.projection_layer(dec_out.permute(0,2,1))
                dec_out = dec_out.reshape(B, self.configs.c_out*len(self.decomposition_variables), self.pred_len).contiguous()
